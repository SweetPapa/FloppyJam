...
void sfbs_audio_phase(AudioEngine*a,Phase p,bool failed,float recovery){recovery=sfbs_clampf(recovery,0,1);a->stem_target[0]=failed?.62f:.78f+.22f*recovery;a->stem_target[1]=failed?0:p>=PH_FLOW?(.72f+.28f*recovery):p>=PH_PULSE?(.58f+.34f*recovery):p>=PH_SIGNAL?(.22f+.24f*recovery):0;a->stem_target[2]=failed?0:p>=PH_BLOOM?(.72f+.28f*recovery):p>=PH_FLOW?(.50f+.42f*recovery):p>=PH_PULSE?(.18f+.24f*recovery):0;a->stem_target[3]=failed?0:p>=PH_BLOOM?(.68f+.32f*recovery):p>=PH_FLOW?(.45f+.32f*recovery):p>=PH_PULSE?(.32f+.26f*recovery):p>=PH_SIGNAL?(.22f+.20f*recovery):0;a->filter_target=failed?.035f:.14f+.08f*recovery;}
