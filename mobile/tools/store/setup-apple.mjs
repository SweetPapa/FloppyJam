...
const reviewPhone=process.env.MAGLAVA_REVIEW_PHONE;
if(reviewPhone) await apple(`betaAppReviewDetails/${appId}`,'PATCH',{data:{type:'betaAppReviewDetails',id:appId,attributes:{contactPhone:reviewPhone,contactFirstName:'Forrester',contactLastName:'Terry',contactEmail:listing.feedbackEmail,demoAccountRequired:false,notes:'No login, purchases, ads or external services. Tap CLIMB to start. Color buttons select a matching node, not direction. Levels unlock sequentially. Test all controls, checkpoints, hazards and pause/resume. On Mac, the same onscreen controls work with the mouse; keyboard R/B/Y/G or W/S/A/D selects colors and Escape pauses.'}}});
const summary={appId,bundleId,versions:versions.map(v=>({id:v.id,platform:v.attributes.platform})),groups:(await apple(`apps/${appId}/betaGroups`)).data.map(g=>({id:g.id,...g.attributes})),status:'Metadata saved; build upload and beta review still pending'};
