...
void sfbs_audio_phase(AudioEngine*a,Phase p,bool failed,float recovery){recovery=sfbs_clampf(recovery,0,1);a->stem_target[0]=failed?.62f:.78f+.22f*recovery;a->stem_target[1]=!failed&&p>=PH_PULSE?(.48f+.52f*recovery):0;a->stem_target[2]=!failed&&p>=PH_FLOW?(.42f+.58f*recovery):0;a->stem_target[3]=failed?0:p>=PH_BLOOM?(.35f+.65f*recovery):p>=PH_SIGNAL?.12f+.18f*recovery:0;a->filter_target=failed?.035f:.14f+.08f*recovery;}
