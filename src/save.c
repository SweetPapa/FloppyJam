...
void sfbs_save_defaults(SaveData*s){memset(s,0,sizeof*s);memcpy(s->magic,"SBS2",4);memcpy(s->seed,"BAD234",7);s->mode=MODE_FLOW;s->volume=80;s->checksum=sum(s);}
bool sfbs_save_valid(const SaveData*s){return !memcmp(s->magic,"SBS2",4)&&s->seed[6]==0&&s->mode<=MODE_PULSE&&s->volume<=100&&s->language<4&&s->checksum==sum(s);}
