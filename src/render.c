...
#define WORLD_SCALE .54f
Palette sfbs_palette(int n){return palettes[n&7];}
static Vector2 rv(V2 v){return(Vector2){320+v.x*WORLD_SCALE,180+v.y*WORLD_SCALE};}
...
void sfbs_render_game(const Game*g,Transport t,bool debug,bool collision){
 Palette p=sfbs_palette(g->genome.palette);Phase phase=(Phase)g->phrases[g->phrase_index<SFBS_PHRASES?g->phrase_index:SFBS_PHRASES-1].phase;ClearBackground(p.bg);DrawRectangleGradientV(0,0,640,360,p.bg2,p.bg);BeginBlendMode(BLEND_ADDITIVE);particles(g,t,p,phase);decorative_geometry(g,t,p,phase);EndBlendMode();
 float breathe=1+sinf(t.beat_phase*SFBS_PI)*(.008f+.008f*phase/PH_FINAL);Vector2 c={320,180};for(int i=0;i<3;i++)DrawCircleLines((int)c.x,(int)c.y,rings[i]*WORLD_SCALE*breathe,ColorAlpha(p.bg2,.9f));DrawCircleLines((int)c.x,(int)c.y,300*WORLD_SCALE,ColorAlpha(p.edge,.32f));for(int i=0;i<24;i++){float a=2*SFBS_PI*i/24,r0=286*WORLD_SCALE,r1=(i%4?292:298)*WORLD_SCALE;DrawLineEx((Vector2){320+cosf(a)*r0,180+sinf(a)*r0},(Vector2){320+cosf(a)*r1,180+sinf(a)*r1},1,ColorAlpha(p.edge,i%4?.18f:.35f));}float sweep=t.bar_phase*2*SFBS_PI;DrawLineEx(c,(Vector2){c.x+cosf(sweep)*296*WORLD_SCALE,c.y+sinf(sweep)*296*WORLD_SCALE},1,ColorAlpha(p.active,.22f));
 ...
 BeginBlendMode(BLEND_ADDITIVE);player_trail(g,t,p);final_form(g,t,p,phase);EndBlendMode();corruption(g,t,p,collision);
 Vector2 pp=rv(g->player.pos);BeginBlendMode(BLEND_ADDITIVE);DrawCircleV(pp,12+g->player.glow*14,ColorAlpha(p.player,.08f));DrawCircleV(pp,8,ColorAlpha(p.player,.12f));DrawCircleLines((int)pp.x,(int)pp.y,7+18*t.beat_phase,ColorAlpha(p.active,.22f*(1-t.beat_phase)));EndBlendMode();DrawCircleV(pp,5,p.player);DrawCircleLines((int)pp.x,(int)pp.y,8+g->player.pulse_cd*6,ColorAlpha(p.accent,.8f));
